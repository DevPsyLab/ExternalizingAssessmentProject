...
- `higherOrderScale`: the higher-order scale(s) (if any) for the measure; a higher-order scale is a scale that subsumes multiple subscales
...
- `reverseScored`: whether the item is reverse scored; one of:
    - `yes`
    - `no`
- `measure1`: the unique identifier of the measure that the item is from; combines the `measureClass` and the `instrument` into one field; should be the same value as the `measure` in the [`Measures`](https://docs.google.com/spreadsheets/d/1luduZayyVmONdxBCcvrMGVv1vshhHzQ7eEOLg1aaMxE/edit#gid=0) tab of the `Existing Measures and Items` spreadsheet; see the [Data Dictionary for the Measures tab](#dataDictionary-measures) for a detailed description of the column
- `measure1itemNumber`: the number of the item on `measure1`
- `measure1HigherOrderScale`: the higher-order scale of the item on `measure1`; see the [Data Dictionary for the Scales and Subscales tab](#dataDictionary-subscales) for a detailed description of the column
- `measure1Subscale`: the subscale of the item on `measure1`; see the [Data Dictionary for the Scales and Subscales tab](#dataDictionary-subscales) for a detailed description of the column
