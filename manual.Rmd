...
- `instrument`: the lowercase version of the abbreviation
    - should be lowercase with no punctuation; however, this may have to include a disambuating element (such as the author name or year of publication) so that there are no duplicates in the `measure` column
- `measure`: a calculated field that combines the `measureClass` and the `instrument` into one field, separated by an underscore; there should be no duplicates
